 ...
 * stars : int (11)
 */
$bdd = $pdo->prepare("CREATE TABLE personnages(
 ...
 * name : varchar (255)
 */
$bdd = $pdo->prepare("CREATE TABLE types(
        ID INT PRIMARY KEY AUTO_INCREMENT,
        name VARCHAR(100)
)");
$bdd->execute();
...
$show_types->execute();
$type = $show_types->fetchAll(PDO::FETCH_OBJ);
#######################
## ETAPE 4
...
if (!empty($_POST)){
    ...
    $pv = $_POST["pv"];
    $type = $_POST["type"];
    $variable = $pdo->prepare('INSERT INTO personnages (name,atk,pv,type_id) VALUES ("'.$name.'",'.$atk.','.$pv.','.$type.')');
    $variable->execute();
